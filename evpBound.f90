...
subroutine exppre(cc,ns,ew,ne,rinv,n,m)
  ...
  do ii = 1,n-3
    ...
    j = m-1
    i = 2
    rinv(m-2+ii,1)  = -((cc(i,j)+ns(i,j) +ew(i-1,j)+ne(i-1,j))     * y(i,j )     & 
               +  (ns(i,j-1) +ne(i-1,j-1))  * y(i,j-1)    &
               +  (ew(i,j)  +ne(i,j))   * y(i+1,j)    &
               +  ne(i,j-1)   * y(i+1,j-1) )
    ! top boundary
    do i = 3, n-2
      rinv(m-2+ii,i-1)  = -((cc(i,j)+ns(i,j))     * y(i,j )     & 
               +  ns(i,j-1)   * y(i,j-1)    &
               +  (ew(i,j)   +ne(i,j))      * y(i+1,j)    &
               +  (ew(i-1,j) +ne(i-1,j))    * y(i-1,j)    &
               +  ne(i,j-1)   * y(i+1,j-1)  &
    ...
    i = n-1
    j = m-1
    rinv(m-2+ii,n-2)  = -((cc(i,j)+ns(i,j) +ew(i,j) +ne(i,j))     * y(i,j )     & 
               +  (ns(i,j-1) +ne(i,j-1))  * y(i,j-1)    &
               +  (ew(i-1,j) +ne(i-1,j))   * y(i-1,j)    &
               +   ne(i-1,j-1)  * y(i-1,j-1) )
    ...
    ! right boundary
    do j =  m-2,3,-1
      rinv(m-2+ii,n-2+m-1-j)  = -( (cc(i,j)+ew(i,j))     * y(i,j )     & 
               +  (ns(i,j)  +ne(i,j))      * y(i,j+1)    &
               +  (ns(i,j-1)+ne(i,j-1))   * y(i,j-1)    &
               +  ew(i-1,j)   * y(i-1,j)    &
               +  ne(i-1,j)   * y(i-1,j+1)  & 
    ...
    i = n-1
    j = 2
    rinv(m-2+ii,n+m-5)  = -( (cc(i,j)+ ns(i,j-1)+ew(i,j)+ne(i,j-1))     * y(i,j )     & 
      +   (ns(i,j) +ne(i,j))   * y(i,j+1)    &
      +  (ew(i-1,j)+ne(i-1,j-1))   * y(i-1,j)    &
      +  ne(i-1,j)   * y(i-1,j+1)  )
  ...
  do j = 1,nm
    do i = 1,nm
      ...
      if (i == j ) then 
        if (abs(rin(i,j) -1.0) > 1.0e-16 ) then 
          write(*,*) 'fail in pre',i,j,rin(i,j)-1.0
        endif 
      else
        if (abs(rin(i,j) -0.0) > 1.0e-16 ) then 
...
subroutine expevp(cc,ns,ew,ne,rinv,u,tu,f,n,m)
  ...
  integer:: n,m
  real*8,dimension(n,m),intent(in) :: cc,ns,ew,ne
  real*8,dimension(n,m),intent(inout) :: f !!! for test 
  real*8,dimension(n,m),intent(inout) :: u
 ...
 ! test evp 
  u(:,:) = f(:,:)
  u(2:n-1,1) = f(2:n-1,2)
  u(2:n-1,m) = f(2:n-1,m-1)
  u(1,2:m-1) = f(2,2:m-1)
  u(n,2:m-1) = f(n-1,2:m-1)
  u(1,1) = f(2,2)
  u(1,m) = f(2,m-1)
  u(n,1) = f(n-1,2)
  u(n,m) = f(n-1,m-1)
  ...
  ry(:,:) = u(:,:)
  ry(2:n-1,2:m-1) = tu
  ry(2:n-1,1) = tu(1:n-2,1)
  ry(2:n-1,m) = tu(1:n-2,m-2)
  
  ry(1,2:m-1) = tu(1,1:m-2)
  ry(n,2:m-1) = tu(n-2,1:m-2)
  ry(1,1) = tu(1,1)
  ry(1,m) = tu(1,m-2)
  ry(n,1) = tu(n-2,1)
  ry(n,m) = tu(n-2,m-2)
